...
#else
#   include <unistd.h>
#   include <stdio.h> // explicit include often needed for printf
#endif
...
const unsigned char CONS_COLORS_FG[] = {   30,   34,    32,   36,  31,      35,     33,    37,   90,    94,     92,    96,   91,       95,      93,     97};
...
inline int set_text_color(unsigned char foreground, unsigned char  background=0){
    ...
    if (background == 1) {
        // If background is "Windows Blue" (1), use xterm-256 Deep Navy (17)
        // Format: \e[ FG_CODE ; 48;5;17 m
        printf("\e[%d;48;5;18m", CONS_COLORS_FG[foreground]);
    }
    else {
        // Standard ANSI behavior for all other colors
